...
function discoverDatabases() {
  ...
  for (const key of Object.keys(process.env)) {
    ...
    if (match) {
      const upper = match[1];                // e.g. "MYDB1"
      const name = upper.toLowerCase();      // "mydb1"
...
// Cache clients (one per database)
const clientCache = {};
...
async function handleRequest(req, res) {
  ...
  if (method === 'GET' && pathname === '/') {
    return sendJson(res, 200, {
      ...
      endpoints: {
        ...
        createDb: 'POST /api/database/:name  (use Turso dashboard)',
        deleteDb: 'DELETE /api/database/:name  (use Turso dashboard)',
  ...
  // ── Create / Delete database (redirect to dashboard) ──
  const dbMatch = pathname.match(/^\/api\/database\/([^\/]+)$/);
  if (dbMatch) {
    const dbName = dbMatch[1].toLowerCase();
    if (method === 'POST') {
      return sendJson(res, 200, {
        message: 'Create the database in the Turso dashboard, then add the URL and token to Railway environment variables as TURSO_DB_<NAME>_URL and TURSO_DB_<NAME>_TOKEN. The database will appear automatically after you re‑deploy or restart.'
      });
    }
    if (method === 'DELETE') {
      return sendJson(res, 200, {
        message: 'Delete the database in the Turso dashboard. Remove its environment variables from Railway to clean up the listing.'
      });
    }
  ...
  // ── Write / any SQL execution (MULTI‑STATEMENT) ──
  ...
  if (execMatch && method === 'POST') {
    ...
    try {
      // ✅ Use executeMultiple to run the full script
      const results = await client.executeMultiple(sql);
      // Sum all rows affected across all statements
...
http.createServer(handleRequest).listen(PORT, () => {
  console.log(`🚀 Turso‑based server running on port ${PORT}`);
  console.log(`Databases discovered: ${discoverDatabases().map(d => d.name).join(', ') || 'none'}`);
});
