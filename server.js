...
// =====================================================================
// 1. CORS middleware – allow all origins (for development / public API)
// =====================================================================
...
// =====================================================================
// 2. Database setup – use /data volume if available, otherwise root
// =====================================================================
...
if (fs.existsSync(DATA_DIR)) {
  ...
  dbPath = path.join(DATA_DIR, DB_FILENAME);
  // Copy the bundled database to the volume on first run
  const bundledDb = path.join(__dirname, DB_FILENAME);
...
// =====================================================================
// 3. Schema creation (if the database is empty)
// =====================================================================
...
app.get('/api/groups', (req, res) => {
  const rows = db.prepare('SELECT * FROM groups ORDER BY name').all();
  res.json(rows);
});

app.get('/api/stages', (req, res) => {
  const rows = db.prepare('SELECT * FROM stages ORDER BY "order"').all();
  res.json(rows);
});
...
app.get('/api/standings', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM group_standings ORDER BY groupId, rank'
  ).all();
  res.json(rows);
});

app.get('/api/standings/simple', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM group_standings_simple ORDER BY groupName, points DESC, goalDifference DESC'
  ).all();
  res.json(rows);
});

app.get('/api/standings/full', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM group_standings_full ORDER BY groupId, rank'
  ).all();
  res.json(rows);
});

app.get('/api/best-third', (req, res) => {
  const rows = db.prepare('SELECT * FROM best_third_placed').all();
  res.json(rows);
});
...
app.get('/api/matches', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM match_details_sorted ORDER BY sort_date, time'
  ).all();
  res.json(rows);
});

app.get('/api/matches/upcoming', (req, res) => {
  const rows = db.prepare('SELECT * FROM upcoming_matches').all();
  res.json(rows);
});

app.get('/api/matches/finished', (req, res) => {
  const rows = db.prepare(
...
app.get('/api/matches/unified', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM unified_matches ORDER BY sort_date, time'
  ).all();
  res.json(rows);
});
...
app.get('/api/match/:id/events', (req, res) => {
  const rows = db.prepare(
...
app.get('/api/match/:id/timeline', (req, res) => {
  const row = db.prepare(
    'SELECT timeline FROM match_timeline WHERE matchId = ?'
  ).get(req.params.id);
...
app.get('/api/knockout', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM knockout_matches_resolved ORDER BY stageOrder, matchNumber'
  ).all();
  res.json(rows);
});

app.get('/api/knockout/simple', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM knockout_matches_resolved_simple ORDER BY matchNumber'
  ).all();
  res.json(rows);
});

app.get('/api/knockout/flags', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM knockout_with_flags ORDER BY stageId, matchNumber'
  ).all();
  res.json(rows);
});
...
app.get('/api/team/:id/history', (req, res) => {
  const rows = db.prepare(
    'SELECT * FROM team_match_history WHERE teamId = ? ORDER BY date, time'
  ).all(req.params.id);
  res.json(rows);
});

app.get('/api/team/:id/form', (req, res) => {
  const row = db.prepare(
    'SELECT form_last5 FROM team_form WHERE teamId = ?'
  ).get(req.params.id);
  res.json(row || { form_last5: '' });
});

// ---------- Scorers & Cards ----------
app.get('/api/scorers', (req, res) => {
  const rows = db.prepare('SELECT * FROM top_scorers').all();
  res.json(rows);
});

app.get('/api/cards', (req, res) => {
  const rows = db.prepare('SELECT * FROM card_summary').all();
  res.json(rows);
});

app.get('/api/own-goals', (req, res) => {
  const rows = db.prepare('SELECT * FROM own_goals').all();
  res.json(rows);
});

// ---------- Tournament progress ----------
app.get('/api/tournament/progress', (req, res) => {
  const row = db.prepare('SELECT * FROM tournament_progress').get();
  res.json(row);
});

// ---------- Head-to-head ----------
app.get('/api/head-to-head', (req, res) => {
  const rows = db.prepare('SELECT * FROM head_to_head').all();
  res.json(rows);
});
...
// =====================================================================
app.listen(PORT, () => {
  console.log(`Tournament API is live on port ${PORT}`);
  console.log(`Database path: ${dbPath}`);
});

// =====================================================================
// 6. The complete SQL schema (your exact statements)
// =====================================================================
const SCHEMA_SQL = `
...
-- All your views and triggers follow (exactly as you provided)...

${VIEWS_AND_TRIGGERS_SQL}
`;

// Separate constant for the huge views/triggers block to keep things tidy
const VIEWS_AND_TRIGGERS_SQL = `
