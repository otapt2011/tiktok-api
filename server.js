...
async function handleRequest(req, res) {
  // CORS headers – now includes Allow-Headers
  ...
  // ── 4. Execute write queries (new endpoint) ───────
  ...
  if (execMatch && method === 'POST') {
    ...
    const db = new Database(dbPath, { readonly: false });
    try {
      db.exec(sql);                // Run the SQL (may contain multiple statements)
      const changes = db.changes;  // total rows affected by last exec
      db.close();
