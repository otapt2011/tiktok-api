...
async function tursoExecute(dbUrl, dbToken, sql) {
  // Turso’s HTTP query endpoint (single statement)
  const endpoint = `${dbUrl.replace(/\/$/, '')}/v2/query`;
  ...
  return data; // contains "results" with rows/affected, etc.
}
...
async function handleRequest(req, res) {
  ...
  if (queryMatch && method === 'GET') {
    ...
    try {
      ...
      // Turso returns rows inside "results" array – we need the first result set
      const rows = result.results && result.results[0] ? result.results[0].rows : [];
  ...
  if (execMatch && method === 'POST') {
    ...
    try {
      ...
      for (const stmt of statements) {
        ...
        // rowsAffected may be in result.results[0].rows_affected or result.rows_affected
