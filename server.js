...
// ── Turso HTTP API caller (using /v2/pipeline) ────────
async function tursoExecute(dbUrl, dbToken, sql) {
  ...
  // Pipeline response contains an array of results; the execute result is the first one
  const executeResult = data.results?.[0]?.response?.result;
  if (!executeResult) {
    throw new Error('Unexpected Turso response format');
  }
  return executeResult; // { cols: [...], rows: [...], rows_affected: ... }
}
...
async function handleRequest(req, res) {
  ...
  if (queryMatch && method === 'GET') {
    ...
    try {
      ...
      // Extract rows from the execute result
