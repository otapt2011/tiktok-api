...
// ── Turso HTTP API caller (with full error handling) ──
...
function splitSqlStatements(sql) {
  ...
  while (i < sql.length) {
    ...
    // BEGIN/END depth (simple detection)
    if (!inSingleQuote && !inDoubleQuote && !inBlockComment && !inLineComment) {
      if (char === 'B' || char === 'b') {
...
const server = http.createServer(async (req, res) => {
  // CORS headers – set them early for every response
  ...
  const searchParams = parsedUrl.searchParams;   // 👈 correct parameter extraction
  ...
  try {
    ...
    if (queryMatch && method === 'GET') {
      ...
      const sql = searchParams.get('sql');   // 👈 use searchParams
