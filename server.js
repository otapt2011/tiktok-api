...
// ── Smart SQL splitter (fallback when executeMultiple is missing) ──
function splitSqlScript(sql) {
  // Simple split: separate on ';' that is not inside quotes
  // This is safe for DDL scripts that don't contain ';' inside strings.
  ...
  if (execMatch && method === 'POST') {
    ...
    try {
      ...
      if (typeof client.executeMultiple === 'function') {
        ...
        totalChanges = results.reduce((sum, r) => sum + (r.rowsAffected || 0), 0);
      } else {
        // Fallback: split on ';' and run statements one by one
        const statements = splitSqlScript(sql);
        for (const stmt of statements) {
          const r = await client.execute(stmt);
          totalChanges += r.rowsAffected || 0;
        }
      }
